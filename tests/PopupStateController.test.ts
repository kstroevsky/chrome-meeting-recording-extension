...
jest.mock('../src/shared/settings', () => {
  const actual = jest.requireActual('../src/shared/settings');
  return { ...actual, loadExtensionSettingsFromStorage: jest.fn().mockResolvedValue(actual.DEFAULT_EXTENSION_SETTINGS) };
});

import { PopupStateController } from '../src/popup/controllers/PopupStateController';
import { sendToBackground } from '../src/shared/messages';
import { loadExtensionSettingsFromStorage } from '../src/shared/settings';
import type { RecordingStatusView } from '../src/shared/recording';
...
describe('PopupStateController', () => {
  beforeEach(() => {
    ...
    (loadExtensionSettingsFromStorage as jest.Mock).mockResolvedValue(
      jest.requireActual('../src/shared/settings').DEFAULT_EXTENSION_SETTINGS
    );
  });
