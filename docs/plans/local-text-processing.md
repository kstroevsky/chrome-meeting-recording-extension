...
> **How to read this.** The base text is r1 as approved; everything since is an authorized delta in Appendix B, which now runs to **D-27**. Prose that a delta superseded is struck through or marked in place rather than deleted, because the base is read-only under this discipline. Where a struck passage and a delta disagree, **the delta wins** — the three that change the most are D-13 (nothing is fetched; the model is packaged), D-24 (the outbox is IndexedDB, not `chrome.storage`) and D-16 (the 0.82 threshold is inoperative for this encoder). Implementation status per phase is in §10.
...
**Everything is packaged; nothing is fetched.** The payload never named an origin. ~~Weights come from an origin the project controls, not a third-party CDN, and reach `host_permissions` through a build-time define (D-03).~~ **Superseded by D-13:** the model ships inside the extension alongside the runtime, so there is no weights origin, no `host_permissions` entry and no production-build check for one. An ONNX export is a computational graph, not data, and MV3's remote-code rule applies to it.
...
type CaptionUtterance = { startWallMs: number; endWallMs: number; speaker: string; text: string }; // D-08: wall clock, named so
...
## 3. Phase 0 — the transcript has to exist first  *(shipped; see §10 step 2)*
...
**Persistence mirrors the notation aggregate exactly.** A `transcripts` store in the existing `recording-history` database (`RecordingLibraryDatabase.ts`, version 4 → 5, presence-driven idempotent upgrade), a `RecordingTranscriptRepository` implementing a port with a read-modify-write `update()` inside one `readwrite` transaction, and a `RecordingTranscriptService` above it — the same four-layer stack as `RecordingNotationRepository` / `RecordingNotationService`. Protocol follows the house rule that live commands are id-less and history commands are keyed, and the new response types join `NON_SESSION_RESPONSE_MESSAGE_TYPES` so a transcript failure can never kill capture.
...
- **CSP is proven, not presumed (BLD-07).** ~~Do **not** add it speculatively … if nothing is needed, the manifest keeps its current absence of a custom CSP.~~ **Settled by D-15 and the 4A run:** `'wasm-unsafe-eval'` in `content_security_policy.extension_pages` is *required* — Chrome's default extension CSP disables WebAssembly outright — and it is declared. The run proved the packaged stack works *under* that CSP; nothing beyond it was added.
- **Package size is measured, not assumed.** **Measured:** ~118 MB of Q8 weights, ~16 MB of tokenizer and ~74 MB of ONNX Runtime WASM, a 104.6 MB release ZIP; type-checking is 3.7 s of a 10.3 s build, so `transpileOnly` stays off (and `skipLibCheck` turns out to be a *requirement* of the dependency, not an optimization). A later ADR may externalize cryptographically pinned *raw tensor data only* if that proves unacceptable.
- **Build cost risk.** ~~`ts-loader` runs without `transpileOnly`, so a full type-check runs on every build. Measure the delta after adding the dependency; if it is material, that is a separate decision, not a silent switch to `transpileOnly` inside this work.~~ **Closed 2026-09-15, measured:** type-checking is 3.7 s of a 10.3 s build — not material, `transpileOnly` stays off. The measurement also found that `skipLibCheck: true` is a *requirement* of `@huggingface/transformers`, not an optimization: without it the build fails inside the dependency's own `.d.ts`. See ADR-0007's hardening amendment.
...
The payload fixes a lot exactly — `C_new = (nC + x)/(n + 1)`, the 0.70/0.10/0.10/0.10 blend, the 0.30/0.25/0.20/0.15/0.10 ranking, batch 32, 384 dimensions — and those are carried verbatim. (0.82 was on this list until D-16 established it was illustrative.) It does **not** fix the following, and this plan deliberately does not invent them.
...
| ~~Embedding throughput target~~ | — | **Baseline frozen: 4.4 windows/sec** (181 s for three hours). ~~A regression *gate* still needs run-to-run variance — step 9, not a blocker~~ **Gate closed 2026-09-15:** four runs measured 5.2–5.4 (WebGPU) and 4.9–5.6 (WASM) windows/sec, so a **3.5 windows/sec floor** is asserted in `analysis-embedding-bench.spec.ts` — a tripwire ~30% below the slowest observation, sitting below the frozen baseline too |
...
| Window size and stride for contextual windows | **4B** | EMB-06 gives only the 300–800 count for 3 hours. Candidate: 4 utterances, stride 4 — the strongest structural result of D-16 |
...
1. ~~**ADR-0007.**~~ **Done** — `docs/adr/0007-topics-are-derived-from-a-persisted-transcript.md`, `Status: Accepted` since 4A passed. The spikes themselves are step 4 below.
2. ~~**Phase 0 — transcript aggregate.**~~ **Done** (§3). `transcriptStatus` is real; captions persist as media-relative segments.
3. ~~**Pipeline stages.**~~ **Done** (§5) — `src/shared/analysis/`: windows, boundaries, peaks, segments, online clusters, c-TF-IDF labels, importance. MMR is implemented but not wired into the pipeline (D-20). Pure functions against a stub encoder; no GPU, no model, no network. Every §9 value is a required config field with no default anywhere.
4. ~~**4A — Platform spike.**~~ **Done** — ADR-0007 is `Accepted`. Frozen: dtype `q8`, throughput baseline 4.4 windows/sec, `'wasm-unsafe-eval'` declared, all four ORT variants packaged, 104.6 MB release ZIP. Original scope follows.
   ...
   **4B — Quality calibration.** Run the deterministic pipeline over a small corpus of real and synthetic meetings with known topic boundaries and known recurrence, and freeze §9's *semantic* values from how well the output matches: window size and stride, the peak rule, `longPauseMs`, `minSegmentMs`, the merge threshold and period, ~~MMR lambda~~ *(removed from 4B's scope by D-20 — nothing consumes it yet)*, and the provisional definitions of `speakerPatternChange`, `novelty`, `keyword_distinctiveness` and `recurrence`. Needs 4A only for a real encoder; needs no GPU benchmark, and a GPU benchmark could not answer any of it.
   **4B status:** run against a *synthetic* corpus; every semantic value is a candidate. The harness for the real pass is built and waiting for conversations — `tests/fixtures/calibration/README.md` gives the fixture format, `analysis-calibration-dump.spec.ts` embeds them once, and `scripts/calibrate-analysis.ts` searches offline, enforcing the tune/holdout split itself and reporting false merges separately from F1. **Not blocked on Plan B** — it needs real conversational text with hand-labelled boundaries, which any offline transcription tool can supply as fixture `TranscriptSegment[]`. Plan B is a product capability, not a calibration prerequisite.
5. ~~**Build seams.**~~ **Done.** §7 — only what step 4 proved: webpack worker entry and asset copying, the declared CSP, and a production-build check that the model, its runtime and its identity all shipped. The worker tsconfig is deferred (D-26): `WebWorker` and `DOM` cannot share a `lib`, so the worker hand-declares its global as `opfsWorker` does. No weights-origin define, no host permission, no production-build check for either (BLD-02/BLD-03 superseded).
6. ~~**Embedding engine.**~~ **Done.** `EmbeddingWorkerClient` + `analysisWorker`, WebGPU/WASM ladder, batch 32. Exit criterion: 384-dim vectors out, WASM fallback exercised by forcing the latch.
7. ~~**Analysis job and results.**~~ **Done**, then corrected twice under review (D-20…D-25). `AnalysisManager` + durable outbox + ack + `closeForUpdate` refusal + the `analyses` store, every row carrying `AnalysisProvenance` (PROV-01). Exit criterion: kill the service worker mid-analysis and have the job complete and report; a stored result computed under different conditions is recognized as stale.
8. ~~**Surface.**~~ **Done.** §8 — manifest topics, scrub band, TOPICS popover, `T`/`⇧T`, library search.
9. ~~**Hardening.**~~ **Done, except** the multilingual pass on a *real* non-English recording (run on synthetic transcripts only). Quota behaviour, throughput variance, both durability E2E tests, and per-module READMEs for the new directories (`src/offscreen/analysis/`, `src/shared/analysis/`) per `docs/agents/module-readmes.md`.
...
5. ~~**Durability:** kill the service worker mid-analysis; the job completes in offscreen, replays its terminal state on reconnect, and the sealed recording is never touched. Attempt an extension update mid-analysis; `closeForUpdate()` refuses.~~ **Closed 2026-09-15, extended 2026-09-17** — `tests/e2e/analysis-durability.spec.ts` carries **two** tests against a real MV3 worker: one kills it *during* embedding, the other holds the completed-but-unstored window open with a blocking IndexedDB transaction and kills it there, asserting the result was **redelivered** (~0.5 s) rather than recomputed (~27 s). Both fail when their fix is reverted. The `closeForUpdate()` refusal and every other reload path are covered by unit tests in `OffscreenManager.test.ts` and `tests/background.test.ts` rather than E2E; driving a real extension update mid-analysis has no harness seam.
...
- **SEG-06** `active` *(extended by D-18)* — Discourse cues. The payload's exact English set — `"anyway"`, `"by the way"`, `"next question"`, `"moving on"`, `"another thing"`, `"speaking of..."` — is unchanged, and Russian and Ukrainian equivalents are added beside it. An English-only set contributes zero on essentially every call for this project's own user, making the degraded 0.70/0.10/0.10/0.00 blend the rule rather than the exception.
...
- **CLU-04** `active` *(amended by D-16)* — Assignment rule for a new segment embedding `x`: compute `s = cosine(x, C)`; if `s` exceeds a **model- and calibration-specific `assignmentThreshold`** add it to the cluster and update the centroid; otherwise it is a **potential new cluster**. The payload's **0.82** was illustrative, not normative, and is measured inoperative for `multilingual-e5-small` (ADR-0007 4B: 27 of 27 cross-topic pairs above it, lowest 0.861). The threshold is an open contract (§9), calibrated **jointly with the merge threshold** because similarity distributions are model-specific. Retained in code as `PAYLOAD_ASSIGNMENT_THRESHOLD` for provenance, unused.
...
- **TX-03** `active` *(qualified by D-17)* — Initially populated from Meet captions only; **no dependency** on Plan B's PCM sidecar or STT work.

  **Known coverage gap, reported by the author 2026-09-12:** Google Meet captions do not work usefully in Russian or Ukrainian, which are the primary languages of this project's own user. For those calls the caption source yields nothing, so Phase 0's aggregate stays empty and no topic analysis can run at all. This does not invalidate the seam — `TranscriptSource` already carries `'stt'`, and the analysis pipeline never learns where words came from (TX-05) — but it does mean the caption fast-path is not a sufficient source for the primary user, and `docs/plans/portable-transcription.md` §B1–B2 (audio sidecar plus local STT) moves from "later, demand-driven" to load-bearing.
...
- **HOST-03** `active` *(amended by D-24, D-25)* — Analysis job terminal state is held in a durable outbox and released only on background ack, mirroring `UploadJobStateOutbox` + `OFFSCREEN_ACK_UPLOAD_STATE`. The outbox lives in **IndexedDB** (`analysis-job-outbox`), not `chrome.storage.local`, which an offscreen document does not have (D-24). The held *result* is released on ack only, never on delivery, and the durable row is removed before the result is (D-25).
- **HOST-04** `active` *(amended by D-25)* — `OffscreenManager.closeForUpdate()` must refuse while an analysis job is active, mirroring its existing refusal for `activeUploadJobs`. "Active" includes a **completed job whose result is unacknowledged**, and the refusal applies to **every** reload path — `onUpdateAvailable` and the deferred reload too — through one definition of critical work, which also keeps the service worker alive so Chrome cannot install a pending update by unloading it.

### BLD — build, CSP and weight delivery *(added by D-03, D-06)*

- **BLD-01** `active` *(amended by D-26)* — Engine code (Transformers.js and ONNX Runtime Web, including `.wasm`) is **packaged**. ~~Only model weights are fetched, as data.~~ Nothing is fetched: the weights are packaged too (BLD-06, D-13).
...
- **BLD-04** `active` *(amended by D-13)* — `static/manifest.json` gains `content_security_policy.extension_pages` carrying `'wasm-unsafe-eval'`. ~~Added only if the spike proves it necessary, never speculatively (BLD-07).~~ Required, not conditional — see BLD-07 as amended by D-15 (D-26).
- **BLD-05** `active` *(amended by D-26)* — webpack gains ~~`experiments.asyncWebAssembly`,~~ a `.wasm` rule **with `emit: false`** (ONNX Runtime loads its own binaries from `wasmPaths`, so webpack must not), `.mjs` in `resolve.extensions`, and worker-safe chunk output (`chunkLoading: 'import-scripts'`, `publicPath: '/'`). ~~The worker gets its own tsconfig with `WebWorker` in `lib`.~~ The worker tsconfig is **deferred**: the worker hand-declares its global, as `opfsWorker` does, because `WebWorker` and `DOM` cannot share a `lib`.
...
| D-19 | AMEND | §6 prose only | Implementation finding (2026-09-12): §6 specified embeddings "stored as a packed `ArrayBuffer` rather than JSON arrays". Measured: neither `Float32Array` nor `ArrayBuffer` survives the structured clone under `fake-indexeddb`, both returning featureless objects, so a packed row could not be verified by any unit test. Durable rows carry plain number arrays instead — roughly double the bytes against `unlimitedStorage`, in exchange for a format that round-trips identically everywhere and is covered by tests. The in-memory type is unchanged. |
| D-18 | ADD | Extends SEG-06 and IMP-04 | Author authorization (2026-09-12), following D-17. Russian and Ukrainian members are added to both sets; every English member the payload specified is retained unchanged, so this extends rather than replaces. Also fixes a latent defect the extension exposed: both word-boundary checks used `/[a-z0-9]/`, which treats every Cyrillic letter as a boundary — "кстатиь" would have matched the cue "кстати". Both are now `/[\p{Letter}\p{Number}]/u`. |
| D-17 | ADD | Qualifies TX-03; raises the SEG-06 / IMP-04 language gap | Author report (2026-09-12): Meet captions do not work in Russian or Ukrainian, the primary languages spoken. Consequences: (a) the caption source covers little of this user's real usage, promoting Plan B's audio STT from optional to load-bearing; (b) the English-only cue set (SEG-06) and signal set (IMP-04) contribute **zero** by default rather than in an edge case, so the boundary blend runs 0.70/0.10/0.10/0.00 and the ranking 0.30/0.25/0.20/0.15/0.00 for most real calls. Extending both sets to Russian and Ukrainian is proposed but **not** applied — both are exact contracts and adding members needs authorization. The embedding layer itself is unaffected: `multilingual-e5-small` handles both languages, as the 4A agreement sample confirmed. |
| D-16 | AMEND | CLU-04; §9 | Author authorization (2026-09-12): the payload wrote "`s > 0.82`, for example" — illustrative, not normative — and a later ledger edit hardened it into an exact invariant, which was too strong. Measurement makes the correction necessary: with `multilingual-e5-small` every observed cross-topic segment pair scores above 0.82 (lowest 0.861), so preserving the number would preserve a number while defeating the architecture it illustrated. `assignmentThreshold` moves into `ClusterConfig` — and therefore into `configHash` — and is calibrated jointly with the merge threshold. Also corrects the boundary-context invariant: the two contexts compared **across one candidate boundary** must not share utterances; that is not a universal rule that all windows be disjoint. |
...
| D-24 | AMEND | HOST-03 | Measurement (2026-09-17): an offscreen document's `chrome` object exposes `runtime` only — `chrome.storage` is absent. The outbox, written against `chrome.storage.local`, had therefore never stored anything: `platform/chrome/storage.ts` degrades to a no-op when the area is missing, so every write *succeeded* and stored nothing (corrected 2026-09-18 — it does not throw), and the first durability E2E's "no leftover rows" assertion passed vacuously. The outbox moves to IndexedDB, which belongs to the extension origin and so is shared by the offscreen document, the service worker and extension pages. It is a database of its own so background remains the sole writer of `recording-history`. **`UploadJobStateOutbox` (ADR-0004, on `main`) has the same defect and is out of this plan's scope.** |
| D-25 | AMEND | HOST-03, HOST-04; §4 prose | Second review (2026-09-17). (a) Every reload path reads one definition of critical work that includes analysis, and a deferred reload is re-evaluated on analysis transitions, which change nothing in `RecordingSession`. Before an update is applied the data plane is asked directly, because background's view is empty after a restart. (b) A `completed` job holds its recording until its result is stored or discarded. (c) The deletion fence is the durable history **tombstone**; an absent row is *not* a deletion, because analysis is queued at `markStopping`, before finalize creates the row. (d) On acknowledgement the outbox row is removed before the held result. (e) A `completed` row replayed without its result is reported as a lost result and re-run from the transcript. (f) Enqueue-time provenance travels with the job and returns with the result, with the backend stamped on as a diagnostic that `isStale` does not compare. (g) `PIPELINE_VERSION` is 2, for D-22 and D-23. (h) A **discarded** run is announced as such, so its transcript is not swept and it is not analysed. |
| D-26 | AMEND | BLD-01, BLD-04, BLD-05 | Reconciliation (2026-09-17): the three entries still described the pre-D-13 design — fetched weights, a conditional CSP, `experiments.asyncWebAssembly`, and a worker tsconfig — none of which is what was built. Corrected to the implementation; the worker tsconfig is deferred rather than claimed. |
| D-27 | AMEND | HOST-03, HOST-04 | Third review (2026-09-17): two lifecycle races. (a) A `lostResult` was acknowledged and released before its replacement was queued, so a deferred reload could be applied in the gap and destroy the analysis; the unacknowledged outbox row is now a recovery token, released only once recovery is established, and counts as active work until then. (b) A data plane that could not be asked set `pendingReload` without starting the keep-alive, so Chrome could unload the idle worker and install the update anyway; "unknown" is now explicitly critical work, retried and cleared when the data plane speaks. Also: terminal state is sealed before a result is delivered, bounded at three attempts so a broken store cannot freeze the runtime. |
| D-28 | AMEND | HOST-03 | Fourth review (2026-09-17): the degraded sealing path delivered an unsealed result, then acknowledged it by asking the same broken store to remove a row that was never written — so the payload was never released and the runtime stayed busy forever, defeating the liveness the path exists for. Sealing becomes one outcome per job, shared by the terminal-state report and the delivery (which also corrects three attempts having become six across the two call sites), and acknowledgement follows that outcome: unsealed releases without touching the outbox, everything else takes the row-first path. |
