...
describe('RecordingTimer', () => {
  ...
  it('renders banked + live elapsed and ticks once per second while recording', () => {
    ...
    expect(el.textContent).toBe(formatClock(5_000));

    jest.advanceTimersByTime(1_000);
    expect(el.textContent).toBe(formatClock(6_000));

    jest.advanceTimersByTime(2_000);
    expect(el.textContent).toBe(formatClock(8_000));
  });
  ...
  it('shows only the banked time and does not tick while paused', () => {
    ...
    expect(el.textContent).toBe(formatClock(7_000));

    jest.advanceTimersByTime(5_000);
    expect(el.textContent).toBe(formatClock(7_000)); // frozen — no live span
  });
  ...
  it('does not run a live span outside the recording phase', () => {
    ...
    expect(el.textContent).toBe(formatClock(3_000));
  ...
  it('stops ticking after stop() and is idempotent', () => {
    ...
    jest.advanceTimersByTime(1_000);
    expect(el.textContent).toBe(formatClock(1_000));

    timer.stop();
    timer.stop(); // idempotent — must not throw or re-arm
    jest.advanceTimersByTime(5_000);
    expect(el.textContent).toBe(formatClock(1_000)); // frozen after stop
  });
