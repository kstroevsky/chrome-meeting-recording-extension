...
- **Interrupted run** (n4) — a run that ended without the user asking gets its own view, driven by `RecordingStatusView.interruption`. The capture is already sealed and saved by then, so this reports a completed save rather than gating one: it says what happened, where the capture reached, and which notes were kept (the run-sealed one shows `ENDED AT` rather than a length). `Done` clears the notice; `Discard` deletes the recording it produced, which is an ordinary delete, not an abandonment.
- **Discard confirm** (n3) — `ConfirmDialog` takes an optional `details` list so a destructive prompt can *name* what it destroys rather than describing it abstractly. Capped at three lines plus a `+N more`, because a long list would push the actions off a 300px popup.
- **Recordings-list notes** (n1) — each row carries a count chip and a lazily-loaded spoiler. Counts arrive from one `LIST_RECORDING_NOTATION_COUNTS` read for the whole page rather than one per row; a row's notes are fetched only when its chip is first tapped.
- **Notes ribbon** (`RecordingNotesView`) — the same 1 s cadence, but started only while a span is *open* (a closed span does not move). The track **is** the recording so far: it is scaled to elapsed time with the playhead pinned to its right edge, so there is never empty track ahead of "now" and the spans slide left as the meeting gets longer. An earlier build scaled to 82% to leave headroom; the design now pins right on every live card, and because elapsed time is frozen while paused, the scale freezes with it — the spans hold still rather than jumping. Paused has its own amber, `--paused` (d3's `#b07d3c` light / `#d99a45` dark), used by the paused timer, the header dot and label, the held span and the HELD line — a step warmer than the notes gold, so a paused run never reads as a note. The interaction rule the module exists to enforce is that **ending a note is silent** — starting or ending a span opens nothing, and a message is written only by clicking a span. `toStatusView` drops the **live** session's `historyId`, so the popup has no id for the run that is currently recording (it does receive one for finished runs, on `UploadJob.historyId`). Hence the active-run message family (`MARK_NOTATION`, `END_NOTATION`, `LIST_ACTIVE_NOTATIONS`, `UPDATE_ACTIVE_NOTATION`, `REMOVE_ACTIVE_NOTATION`), which follows the line the protocol already draws: **live commands are id-less** (`SET_PAUSED`, `SET_MIC_MUTED`, `STOP_RECORDING` all name no run) **and history commands are keyed**. The ⌥M tip under the notes row shows until the run has a note; its ✕ (e5's *Got it*) retires it for good, remembered in `localStorage` (`meetRecorder.noteHintDismissed`) so the first paint never flashes it back.
...
An upload tab is one screen in four states, set on `#view-upload[data-state]` by `uploadJobPanel`: **uploading** (d4 — each file on its own row as SAVED, `UPLOADING n%` or QUEUED, from the per-file `UploadJobFile.uploadedBytes` the offscreen finalizer reports with every progress tick; the notes sidecar's band says SAVED FIRST; one footer action, *Keep uploading in background*), **saved** (n2a — the Drive block with sizes and open arrows, the folded notes, the transcript row, *Open in Google Drive*), **failed** (8A) and **partial** (8C), which lead with a mono outcome line instead of the head and keep their actions inline. The screen's own header says SAVING / SAVED. The recording's length and note count arrive after the first paint (`PopupController.loadSavedExtras`) and are appended to the summary line. Retry is only offered while the offscreen runtime still retains the failed artifacts; retention is bounded to the latest failed job, five minutes, and 128 MB. Cancel — a ⋯ menu row while an upload is on screen, since d4 keeps the screen to one action — aborts queued or active Drive work and routes unfinished files through the normal local-download fallback. Both actions reconcile from the response/session update rather than assuming a local tab mutation succeeded.

### The transcript as a file

A recording's transcript is rendered to WebVTT at stop and delivered like the notes sidecar (`RecordingController.buildTranscriptSidecar` → `OFFSCREEN_STOP.transcriptSidecar` → an artifact of `kind: 'transcript'`). Both are kilobytes, so both go up before the media: the uploading screen's line names what went first — `7 notes + transcript` (d4) — and the saved screen lists the transcript with its weight, `VTT · 34 KB`, opening where it landed (n2a). A recording captured without captions has no transcript file, and neither line appears.

### Naming completed Drive uploads

**A recording a crash left behind (8D).** Capture writes straight to OPFS, so the bytes survive a tab closing, a worker dying or a power cut — what was missing was being asked about them. `UnsavedRecordingPrompt` offers the newest one back on popup open: a name, **Save it now** to wherever settings says recordings go, or **Discard it**. Asking costs one `chrome.storage.local` read in the ordinary case, because `captureMayBeUnsaved` is set when capture begins and cleared when the run reaches idle; only a set flag makes background create the offscreen document to scan. Declining leaves the recording exactly where it is — the unattended recovery that downloads it without asking takes over only after a week.

When the oldest completed job still has `namingStatus: 'pending'`, `PopupController` selects that job and opens `RecordingNameDialog` after the current render. Save sends `RENAME_RECORDING_HISTORY`; for a Drive entry the background renames the remote folder and every available uploaded file before returning the updated history/session projections. *Keep the default name* sends `SKIP_RECORDING_NAMING`, which durably marks the job handled so reopening the ephemeral popup does not ask again. The dialog follows 9L/9FL: the recording's summary under the title, NAME with the location hint under it (or FOLDER when destinations exist), and a save that is blocked the moment the field empties (7B). Opened, FOLDER's list takes the field's place in the flow rather than floating over the buttons (9FD), the picked folder tinted and ticked; past eight folders it gains a search row and a `4 OF 26 FOLDERS` count (7D), through the shared listbox's opt-in `search`. The list's last row is *New folder…* (7A): it turns into a field in place, and what you type is written to the Settings folder list and selected. Deliberately not a `role="option"` — the search row never filters it away and arrow-key selection never lands on it, because it is an action after the choices rather than one of them. It is hidden when the caller passes no `onCreate`, so a picker that cannot create does not offer to. When the typed name is one the library already has, the field takes the caution tone and a line under it says what this one becomes — `Team sync — Jul 11 (2)` (7C). A warning, not a refusal: two meetings really do share a name, so saving keeps both, and the name it promised is the name it saves. Checked against local history rather than Drive — the same answer, and it works offline, which is exactly when a recording finishes. The recording-detail screen renames in its own header instead (7H): the name becomes the field, Enter saves and Escape keeps the old name. It validates a nonblank, slug-compatible title, traps focus, blocks dismissal while the remote update is active, and surfaces rollback errors without inventing local success.
...
| `PopupStatusView.ts` | header phase/tone (REC, PAUSED, SAVING, SAVED with its own dot, STOPPED), REC banner, chips, finalizing checklist, upload-return control |
