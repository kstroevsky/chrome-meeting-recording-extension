/**
 * @file popup/RecordingDetailView.ts
 *
...
export class RecordingDetailView {
  ...
  render(): void {
    ...
    else this.renderUpload(content, target.job);
  ...
  private renderUpload(content: HTMLElement, job: UploadJob): void {
    const title = document.createElement('h2');
    title.className = 'recording-detail-title';
    ...
    cancel.addEventListener('click', () => void this.cancelUpload(job, cancel));
